...
export default function GroupChatPage() {
  ...
  }, [currentUser, groupId, fetchMessages]);
    ...
    <div className="mx-auto flex max-w-3xl flex-col h-full px-6 py-8 mt-20">
      <div className="flex-1 overflow-y-auto space-y-2 mb-4">
        {hasMore && messages.length > 0 && (
          <button onClick={() => loadMore({ groupId, before: messages[0].createdAt })}
        ...
        {messages.map(m => (
          <div key={m.id} className={`flex ${m.senderId === currentUser?.id ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[70%] px-3 py-2 text-sm ${m.senderId === currentUser?.id ? 'bg-[#CB6CE6] text-white' : 'bg-zinc-100 text-zinc-800'}`}>
              <p className="text-[10px] font-semibold text-zinc-500 mb-0.5">{m.senderId}</p>
              {m.content}
            </div>
          </div>
        ))}
