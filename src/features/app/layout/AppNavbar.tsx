...
import { appNavbar } from './AppNavbar.styles';
...
export function AppNavbar() {
  ...
  return (
    <header className={appNavbar.header}>
      <div
        ref={notchRef}
        className={`${appNavbar.notch} ${appNavbar.notchGlass} max-w-[95vw] sm:max-w-none`}
      >
        <nav className={appNavbar.nav}>
          <div className="flex items-center gap-2 min-w-0 flex-wrap justify-center">
            <Link href="/" className={appNavbar.logo}>
              <svg
                viewBox="0 0 1280 1024"
                className="h-5 w-5"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                aria-label="Peerly logo"
              >
            ...
            <div className={appNavbar.divider} />

            <button
              onClick={() => router.back()}
              className={appNavbar.navBtn}
              aria-label="Go back"
            >
              <ArrowLeft size={14} />
            </button>
              ...
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    key={item.href}
                    href={item.href}
                    className={isActive ? appNavbar.linkActive : appNavbar.link}
                  >
                    <Icon size={14} />
                    {item.label}
                  </Link>
          ...
          <div className="flex items-center gap-2 shrink-0">
            {isSignedIn || currentUser ? (
              <div className="flex items-center gap-2">
                {currentUser && (
                  <div className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium text-white/70 bg-white/5 shadow-[inset_0_1px_0_rgba(255,255,255,0.08)] border border-white/[0.06]">
                    {currentUser.imageUrl ? (
                      ...
                      <div className="flex h-5 w-5 items-center justify-center text-[8px] font-bold text-white bg-[#CB6CE6]/40">
                        {currentUser.avatar || '?'}
                      </div>
                    )}
                    <span className="hidden sm:inline">{currentUser.name || 'Anonymous'}</span>
                  </div>
                )}
              ...
              <div className="flex items-center gap-1.5">
                <SignInButton mode="modal">
                  <button className="px-4 py-2 text-xs font-medium text-white/70 transition-all hover:text-white active:scale-[0.97]">
                    Sign in
                  </button>
                </SignInButton>
                <SignUpButton mode="modal">
                  <button
                    className="px-5 py-2 text-xs font-bold text-white transition-all active:scale-[0.97] shadow-[0_0_20px_rgba(203,108,230,0.15)]"
                    style={{
                      background: '#CB6CE6',
